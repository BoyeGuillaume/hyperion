...
int main(int argc, char **argv) {
  ...
  createInfo.enabledExtensionsCount =
  ...
  result =
      hyCompileModule(instance, &compileInfo, &compiledData, &compiledDataLen);
...
void callback_function(struct HyLogMessageEXT *message) {
  printf("%s%s[%s:%u] -- %s\n" COLOR_RESET, log_level_to_color(message->level),
         log_level_to_string(message->level), message->file, message->line,
         message->message);
}