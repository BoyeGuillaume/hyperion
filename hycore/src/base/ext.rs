...
/// Handle passed to plugin entry/teardown callbacks so they can register
/// resources (such as Python opaque object loaders) with the host.
...
impl LibraryBuilderPtr {
    /// Builds a new handle wired to the global host state that extensions are
    /// allowed to mutate.
    fn create() -> Self {
...
/// Generates all host-facing entry points (`compat`, `entrypoint`, `loader`,
/// `teardown`) that every plugin shared object must export.
///
/// The macro parses the provided compatibility literal into a
/// `semver::VersionReq`, emits the canonical `__hyext_fn_compatibility_check`
/// symbol, forwards entry/teardown notifications to the supplied functions, and
/// wires each `PluginExtStatic` implementation to its UUID inside the loader
/// trampoline. The host will probe these functions immediately after dlopening
/// the shared object.
#[macro_export]
...
pub struct PluginExtWrapper {
    /// Boxed trait object implementing the plugin logic.
...
/// Wrapper around [`libloading::Library`] that calls the plugin tear-down hook
/// when the host drops its last strong reference to the shared object.
...
/// Loads the shared object referenced by `ext_info`, runs the entry-point and
/// compatibility hooks, and memoizes the result so subsequent callers reuse the
/// same [`LibraryWrapper`].
///
...
/// Keeps preloaded shared objects alive for the duration of a critical section.
/// When the guard is dropped the reference counts fall back to whatever the
/// host already holds.
pub struct PluginPreloadGuard {
    /// Libraries kept alive for as long as the guard is in scope.
    _libraries: Vec<Arc<LibraryWrapper>>,
}

/// Preloads the libraries backing the provided extensions names so that Python
/// bridges can resolve opaque objects before instance creation kicks off.
...
/// Entry point used by the host to materialize a specific extension identified
/// by its metadata `name`. Consumes configuration objects from `ExtList`, keeps
/// the shared object alive, and returns a wrapper ready to be attached to an
/// [`InstanceContext`](crate::base::InstanceContext).
